...
const std::int32_t Tensor::get_shape().size() const { return dims_.size(); }
...
const std::int32_t Tensor::get_shape().at(std::int32_t idx) const {
  assert(idx >= 0 && idx < static_cast<int>(dims_.size()));
  return dims_[idx];
}

const std::vector<std::int32_t>& Tensor::get_shape() const { return dims_; }
