...
DWConvFix<DType>::DWConvFix(const xir::Subgraph* subg, const xir::Op* op,
                            IMapTBs_t inputs, CPUTBPtr_t output)
    : DWConv<DType>(subg, op, inputs, output) {
...
template <typename DType>
void DWConvFix<DType>::run() {
  this->dwconv();
  fix();
}

template <typename DType>
void DWConvFix<DType>::print_param() {
  DWConv<DType>::print_param();
...
void DWConvFix<DType>::fix() {
  ...
  for (auto i = 0; i < fmap_o_.num(); i++) {
    ...
    data_out_ptr_[i] = DPURound<DType>(tmp, CPUOPBase::data_min_, CPUOPBase::data_max_);
  }
}

INSTANTIATE_TPCLASS(DWConvFix);
REG_OP_INSTANCE_FUNC("depthwise-conv2d-fix", DWConvFix);
