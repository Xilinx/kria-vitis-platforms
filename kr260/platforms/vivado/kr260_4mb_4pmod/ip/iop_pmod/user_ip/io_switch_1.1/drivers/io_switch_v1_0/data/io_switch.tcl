...
puts "#####write_address_param: periph $periph canon $canon name $name cell $cell"
	if { $cell == ""} {
	puts "#####write_address_param: empty cell, skipping this entry."
	puts "#####write_address_param: probably caused by uart1 not being in same block design container"
	} else {
...
puts "#####write_address_conditional: periph $periph canon $canon pin $pin config $config name $name"
