...
TDWConvFix<DType>::TDWConvFix(const xir::Subgraph* subg, const xir::Op* op,
                              IMapTBs_t inputs, CPUTBPtr_t output)
    : TDWConv<DType>(subg, op, inputs, output) {
...
template <typename DType>
void TDWConvFix<DType>::print_param() {
  DWConvBase<DType>::print_param();
...
template <typename DType>
void TDWConvFix<DType>::check_param() {
...
template <typename DType>
void TDWConvFix<DType>::run() {
  this->dwconv();
  this->fix();
}

template <typename DType>
void TDWConvFix<DType>::fix() {
  // do fix for bias
  if (has_bias_) {
...
INSTANTIATE_TPCLASS(TDWConvFix);
REG_OP_INSTANCE_FUNC("transposed-depthwise-conv2d-fix", TDWConvFix);
